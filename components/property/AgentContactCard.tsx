...
export async function AgentContactCard({ agent }: { agent: NonNullable<PropertyDetail["agent"]> }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-6">
      <Link href={`/agents/${agent.slug}`} className={cn("flex items-center gap-3 rounded-lg", FOCUS_RING)}>
      ...
      <div className="flex flex-col gap-2">
        {agent.phone && (
          <a
            ...
            className={cn(
              "flex h-11 items-center justify-center gap-2 rounded-full bg-accent text-sm font-medium text-accent-foreground transition-opacity hover:opacity-90",
              FOCUS_RING,
            )}
          >
        ...
        {whatsappNumber && (
          <a
            ...
            className={cn(
              "flex h-11 items-center justify-center gap-2 rounded-full border border-border text-sm font-medium text-foreground transition-colors hover:border-accent",
              FOCUS_RING,
            )}
          >
